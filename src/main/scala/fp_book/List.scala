...
def sum(ints: List[Int]): Int = ints match { // A function that uses pattern matching to add up a list of integers
  ...
  def apply[A](as: A*): List[A] = {
   println("apply is called "+as.length)
  ...
  def getListLength[A](l: List[A]): Int = {
  length(l)
    }
  /*
  ...
  */
  def tail[A](l: List[A]): List[A] =
    l match {
      ...
      case Cons(_,t) => {
        println("trail is called inside tail[A]")
        t
      }
    }
...
object run {
  def main(args: Array[String]): Unit = {
    // println(List.sum(List(5,5,5)))

    val x = List(10, 20, 30, 40, 50) match {
      case Cons(x, Cons(2, Cons(4, _))) => {
        println("Cons(x, Cons(2, Cons(4, _)))")
        x
      }
      case Nil => 42
      case Cons(x, Cons(y, Cons(3, Cons(4, _)))) => {
        println("Cons(x, Cons(y, Cons(3, Cons(4, _))))")
        x + y
      }
      case Cons(h, t) => {
        println("Cons(h, t)")
    ...
    val tailLocal: List[Int] = List(1, 2, 3, 4, 5)
    List.tail(tailLocal)
    println("Length is "+ getListLength(tailLocal))
//    List.reverse(tailLocal)
//    println(tailLocal)
  }
}
